...
### ข้อมูล (ตัวเลขจริง ณ 2026-09-12, `src/data/`)

| ไฟล์ | ปริมาณ |
|---|---|
| `lessons.ts` | 18 บท · 374 คำศัพท์ · 188 วลี · 156 บรรทัดสนทนา · 130 โน้ต |
| `sentences.ts` + `sentencesExtra.ts` + `sentences16.ts` + `sentences17.ts` + `sentences18.ts` | 546 ประโยคเรียงคำ (421 + 27 คำ HSK 1 + 29 บทที่ 16 + 30 บทที่ 17 + 39 บทที่ 18 ซึ่งรวมใบงานครู 第16-18课 ส่วนที่ 3 อีก 9 ข้อ) · 11 หมวด (`sentenceCats`) — เขียนมือทั้งหมด ไม่ได้ derive จากบท · รวมกันที่ `allSentences` ใน `lib/pools.ts` |
...
(บทที่ 16 ย้าย 现在 · 时候 · 电影 · 电视 · 少 · 东西 ออกมา · บทที่ 17 ย้าย 想 · บทที่ 18 ย้าย 飞机 ออกมา → `hsk1Extra` เหลือ 37 คำ)
...
คีย์ที่เพิ่มใน v2: `xy-dlsn` (บทที่กรองในแบบฝึก — **คีย์เดียวใช้ร่วมทั้ง 3 แบบฝึก** เลือกบท 17 ที่ควิซแล้วเดินไปเรียงประโยคก็ยังเป็นบท 17) · `xy-sbmode` (วิธีเล่นเรียงประโยค `guided` / `free`)

### กรองแบบฝึกตามบท — `src/lib/lsn.ts` + `<LessonPick />`

ควิซ · เรียงประโยค · ฟังแปล มีแถบ "เลือกบท" (ทุกบท / 1–18 / HSK 1) ใช้ `useDrillLsn()` ร่วมกัน
...
- **`sentencesExtra` / `sentences16` / `sentences17` / `sentences18` ติดบทตรงๆ ที่ `allSentences`** ห้ามให้เดา — ประโยคบท 16 ที่ใช้คำ HSK 1 อย่าง 电视 จะถูกจัดไป `hsk1` ผิด และประโยคบท 17 ที่ใช้แต่คำเก่า (坐车去吧) จะหล่นไปบทต้นๆ
...
### แบบฝึกเรียงประโยค — `SentenceScreen` + `src/lib/sentence.ts` + `src/lib/gloss.ts`

ออกแบบใหม่ 2026-09-13 เพราะผู้ใช้เรียงแบบเดิมไม่ได้เลย ("ทำเท่าไหร่ก็ทำไม่ถูกสักที") — เดิมต้องวางครบก่อนถึงรู้ว่าผิด และไม่บอกว่าผิดตรงไหน

- **🤝 โหมดช่วย (ค่าเริ่มต้น)** รับเฉพาะคำถัดไปที่ถูก คำผิดเด้งกลับทันที · ผิด 2 ครั้งติดที่ตำแหน่งเดิม → ชี้คำให้เอง
- **🧠 เรียงเอง** วางครบแล้วตรวจ — ชี้สีแดง **ตั้งแต่คำแรกที่ผิดไปจนสุด** (`goodPrefix`) แตะเอาออกเฉพาะตัวแดง
  ⚠️ ห้ามชี้ผิดเป็นรายตำแหน่ง — ถอดตัวผิดออกแล้วตัวที่เคยถูกตำแหน่งจะเลื่อนมาผิดที่แทน กลายเป็นแดงเองทั้งที่ทำตามที่บอก (เจอตอนทดสอบ)
- **💡 คำใบ้** กระพริบคำถัดไป · ใช้ตัวช่วย/เคยผิด = ข้อนั้นนับผิดและกลับมาถามอีก (`markWrong`)
- ไทล์โชว์ **พินอิน → ความหมายไทย → ฮั่นจื่อ (ตัวเล็ก)** · หลังตอบมีตาราง "คำศัพท์ในประโยคนี้" แตะฟังได้
- ความหมายไทยของ token **ไม่ได้เขียนลงไฟล์ประโยค** — `glossFull()` ดึงจากคลังคำ (คำศัพท์ → HSK 1 → วลี → `EXTRA` ใน `gloss.ts` → แตกทีละตัว) · เทสคุมว่าทุก token ต้องมีความหมาย ถ้าเพิ่มประโยคที่ใช้คำนอกคลัง ต้องเติม `EXTRA`
- `Sentence.alt` = ลำดับอื่นที่ถูกด้วย (เช่นคำบอกเวลาวางหน้า/หลังประธานได้) — **เทียบเป็นลำดับ token ไม่ใช่ string ต่อกัน** ไม่งั้น 我 จะผ่านเป็นคำแรกของ 我们… แล้วโหมดช่วยค้างกลางทาง
...
5. **กวาด overflow ที่ 375px** — ทุกแท็บ + 18 บท (+ HSK 1) × 4 โหมด `document.documentElement.scrollWidth > innerWidth` ต้องเป็น false หมด
