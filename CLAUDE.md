...
| ไฟล์ | ปริมาณ |
|---|---|
| `lessons.ts` | 16 บท · 324 คำศัพท์ · 139 วลี · 117 บรรทัดสนทนา · 100 โน้ต |
| `sentences.ts` + `sentencesExtra.ts` + `sentences16.ts` | 477 ประโยคเรียงคำ (421 + 27 คำ HSK 1 + 29 บทที่ 16) · 11 หมวด (`sentenceCats`) — เขียนมือทั้งหมด ไม่ได้ derive จากบท · รวมกันที่ `allSentences` ใน `lib/pools.ts` |
...
(บทที่ 16 ย้าย 现在 · 时候 · 电影 · 电视 · 少 · 东西 ออกมา → `hsk1Extra` เหลือ 39 คำ)
...
คีย์ที่เพิ่มใน v2: `xy-dlsn` (บทที่กรองในแบบฝึก — **คีย์เดียวใช้ร่วมทั้ง 3 แบบฝึก** เลือกบท 16 ที่ควิซแล้วเดินไปเรียงประโยคก็ยังเป็นบท 16)

### กรองแบบฝึกตามบท — `src/lib/lsn.ts` + `<LessonPick />`

ควิซ · เรียงประโยค · ฟังแปล มีแถบ "เลือกบท" (ทุกบท / 1–16 / HSK 1) ใช้ `useDrillLsn()` ร่วมกัน
...
- **`sentencesExtra` / `sentences16` ติดบทตรงๆ ที่ `allSentences`** ห้ามให้เดา — ประโยคบท 16 ที่ใช้คำ HSK 1 อย่าง 电视 จะถูกจัดไป `hsk1` ผิด
...
5. **กวาด overflow ที่ 375px** — ทุกแท็บ + 16 บท (+ HSK 1) × 4 โหมด `document.documentElement.scrollWidth > innerWidth` ต้องเป็น false หมด
